...
export const onRequestPut: PagesFunction = async (ctx) => {
  ...
  const r = await ctx.env.DB.prepare("UPDATE travelers SET name = ? WHERE id = ?").bind(name, id).run();
  if (r.meta.changes === 0) return err("Traveler not found.", 404);
  const { results } = await ctx.env.DB.prepare("SELECT id, name, created_at FROM travelers WHERE id = ?").bind(id).all();
